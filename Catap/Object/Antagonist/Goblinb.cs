...
namespace Catap.Object
{
    class Goblinb : Enemy
    {
        ...
        public float speedTime = (float)0.0007;
        ...
        public Goblinb(Texture2D texture, World world, int positionx, int positiony, ContentManager _content)
            : base(texture, world)
        {
            _animations = new Dictionary<string, Animation>
                {
                    { "WalkLeft", new Animation(_content.Load<Texture2D>("Walk/4Walk"), 3,0.2f) },
                    { "Attack", new Animation(_content.Load<Texture2D>("Atk/4Attack"), 5,0.2f) }
                };
            _animationManager = new AnimationManager(_animations.First().Value);
            /* Circle */
            stop = ran.Next(10, 40);
            _enemyRadius = 1.5f / 2f;
