...
namespace Catap.Object
{
    class Bossummon : Enemy
    {
        ...
        public Bossummon(Texture2D texture, World world, int positionx, int positiony, ContentManager _content)
            : base(texture, world)
        {
            _animations = new Dictionary<string, Animation>
                {
                    { "WalkLeft", new Animation(_content.Load<Texture2D>("Walk/SummonerWalk"), 5,0.2f) },
                    { "Attack", new Animation(_content.Load<Texture2D>("Atk/SummonerAttack"), 11,0.1f) }
                };
            _animationManager = new AnimationManager(_animations.First().Value);
            /* Circle */
            _enemyRadius = 3.0f / 2f;
