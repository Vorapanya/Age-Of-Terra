...
{
    class Burnaoe : Bullet
    {
        public Burnaoe(Texture2D texture, World world, Vector2 position, ContentManager _content)
            : base(texture, world, position)
        {
            _animations = new Dictionary<string, Animation>
                {
                    { "boom", new Animation(_content.Load<Texture2D>("Eff/Burn"), 5 ,0.1f) },
                };
        ...
        public override void update(GameTime gameTime)
        {
            if(_timer == 0)
            {
                Vector2 v = new Vector2(_bulletBody.Position.X, _bulletBody.Position.Y +1);
                _bulletBody.SetTransform(v, 0);
            }
